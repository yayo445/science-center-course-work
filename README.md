...
## Оновлення новин з сайту університету (парсер)

Свіжі новини тягнуться з офіційного сайту nupp.edu.ua скриптом `parse_news.py`.
...
Скрипт перезапише `data/news.json` 10 останніми новинами (заголовок, дата, фото, повний текст та посилання на оригінал).
Після цього закомітьте і запуште зміни на GitHub. Кількість новин змінюється змінною `COUNT` у `parse_news.py`.
